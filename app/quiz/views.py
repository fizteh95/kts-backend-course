...
class ThemeDeleteView(AuthRequiredMixin, View):
    ...
    async def post(self):
        title = self.data["title"]
        deleted_theme = await self.store.quizzes.delete_theme_by_title(title)
        if not deleted_theme:
            raise HTTPNotFound
        return json_response(data=ThemeSchema().dump(deleted_theme))
...
class QuestionDeleteView(AuthRequiredMixin, View):
    @request_schema(QuestionToDeleteSchema)
    @response_schema(QuestionSchema)
    async def post(self):
        title = self.data["title"]
        deleted_question = await self.store.quizzes.delete_question_by_title(title)
        if not deleted_question:
            raise HTTPNotFound
        return json_response(data=ThemeSchema().dump(deleted_question))
